...
function BuiltCards(){
	...
	for (i = 0; i < Preguntas.DOM.length; i++) {
		...
		allTextParsed.forEach(function(e,index){
			if(addBreak){
				preg+= (enumerator ? ("<h6>" + (i+1) + '.- </h6>'): '') + "<div class='form-group mb-1 pr-1' style='padding-bottom: 18px;'><input type='text' apex='"+e+"' class='badge-normal drag-reciver form-control form-control-sm'></input></div>";	
				addBreak = false;
				enumerator = false;
			}		
			else{ 
	...
	if(!continueNav)
	{
		...
		if(respuestas){
			[].forEach.call(respuestas, function(col) {
				if(col.getAttribute("apex").toLowerCase() === col.value.toLowerCase()){
					good +=1
