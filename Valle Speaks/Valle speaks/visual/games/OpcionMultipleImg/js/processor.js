...
function getNext(){
	try{
		var preg = '<div class="card-deck">';

		Preguntas.DOM[indexer].respuesta.forEach(function(e,i){
			preg += newCard(i, e.img, e.texto)
		});
		...
		$('.radio').on('click', function() {
			if($('.card').hasClass('radio-option-selected'))
				$('.card').removeClass('radio-option-selected')
			$('.card').addClass('radio-option');
...
function newCard(i, img, texto){
	...
	{
		return (img ? 
			'<div class="card radio-option bounceIn" id="aws-rp-'+ i +'">' + 
				'<img class="img-thumbnail" src="'+ header.url +'resources/images/'+img+'" alt="slide">' + 
				'<div class="card-body theme">' + 
					newRadioButton(i, texto) + 
				'</div>' +
			'</div>':
		'')
	}
...
function newRadioButton(i, texto) {
	return 	'<div class="custom-control custom-radio">' + 
				'<input class="radio custom-control-input" type="radio" name="respuesta" id="rp-'+ i +'" value="'+texto +'">' + 
				'<label class="custom-control-label ml-1 label-item" for="rp-'+ i +'"><h5 class="ml-2">'+texto +'</h5></label>' +
			'</div>';
}
